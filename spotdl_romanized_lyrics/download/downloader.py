...
from spotdl_romanized_lyrics.types import Song
from spotdl_romanized_lyrics.utils.ffmpeg import FFmpegError, convert, get_ffmpeg_path
from spotdl_romanized_lyrics.utils.metadata import embed_metadata, MetadataError
from spotdl_romanized_lyrics.utils.formatter import create_file_name, restrict_filename
from spotdl_romanized_lyrics.providers.audio.base import AudioProvider
from spotdl_romanized_lyrics.providers.lyrics import Genius, MusixMatch, AzLyrics
from spotdl_romanized_lyrics.providers.lyrics.base import LyricsProvider
from spotdl_romanized_lyrics.providers.audio import YouTube, YouTubeMusic
from spotdl_romanized_lyrics.download.progress_handler import NAME_TO_LEVEL, ProgressHandler
from spotdl_romanized_lyrics.utils.config import get_errors_path, get_temp_path
from spotdl_romanized_lyrics.utils.search import reinit_song
...
TRANSLATE_PROVIDERS: Dict[str, Type[TranslateProvider]] = {
...
class Downloader:
    ...
    def __init__(
        ...
        translate_providers: Optional[List[str]] = None,
        ...
        translate_providers_classes: List[Type[TranslateProvider]] = []
        ...
        for provider in translate_providers:
            new_translate_provider = TRANSLATE_PROVIDERS.get(provider)
            if new_translate_provider is None:
                raise DownloaderError(f"Invalid translate provider: {provider}")

            translate_providers_classes.append(new_translate_provider)
        ...
        # try to use the spotdl_romanized_lyrics's ffmpeg
        ...
        self.translate_providers: List[TranslateProvider] = []
        for translate_provider_class in translate_providers_classes:
            self.translate_providers.append(translate_provider_class())
    ...
    def translation(self, song: Song) -> Optional[str]:
        for translate_provider in self.translate_providers:
            ...
            if trans:
                self.progress_handler.debug(
                    f"Found translate for {song.display_name} on {translate_provider.name}"
                )
                return trans

            self.progress_handler.debug(
                f"{translate_provider.name} failed to find translate "
                f"for {song.display_name}"
            )
