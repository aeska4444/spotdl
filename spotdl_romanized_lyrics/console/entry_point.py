...
from spotdl_romanized_lyrics.console.download import download
from spotdl_romanized_lyrics.console.sync import sync
from spotdl_romanized_lyrics.console.save import save
from spotdl_romanized_lyrics.console.meta import meta
from spotdl_romanized_lyrics.console.web import web
from spotdl_romanized_lyrics.download import Downloader
from spotdl_romanized_lyrics.providers.audio.base import AudioProviderError
from spotdl_romanized_lyrics.providers.audio.ytmusic import YouTubeMusic
from spotdl_romanized_lyrics.utils.config import DEFAULT_CONFIG, get_config, get_config_file
from spotdl_romanized_lyrics.utils.arguments import parse_arguments
from spotdl_romanized_lyrics.utils.spotify import SpotifyClient, SpotifyError
from spotdl_romanized_lyrics.utils.console import ACTIONS
from spotdl_romanized_lyrics.download.downloader import DownloaderError
...
def entry_point():
    """
    Console entry point for spotdl_romanized_lyrics. This is where the magic happens.
    """
    ...
    if is_ffmpeg_installed(settings["ffmpeg"]) is False:
        raise FFmpegError(
            "FFmpeg is not installed. Please run `spotdl_romanized_lyrics --download-ffmpeg` to "
            "install it, "
            "or `spotdl_romanized_lyrics --ffmpeg /path/to/ffmpeg` to specify the path to ffmpeg."
        )
    ...
    if isinstance(settings["save_file"], str) and not settings["save_file"].endswith(
        ...
        raise DownloaderError("Save file has to end with .spotdl_romanized_lyrics")
    ...
    downloader = Downloader(
        ...
        add_translation=settings["add_translation"],
    )
...
def console_entry_point():
    ...
    if "--profile" in sys.argv:
        ...
        stats.dump_stats("spotdl_romanized_lyrics.profile")
    else:
        entry_point()
