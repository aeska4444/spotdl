"""
Init module for spotdl_rl. This module contains the main entry point for spotdl_rl.
And spotdl_rl class
"""
...
from spotdl_rl.utils.search import parse_query
from spotdl_rl.utils.spotify import SpotifyClient

__all__ = ["spotdl_rl", "console_entry_point", "__version__"]
...
class spotdl_rl:
    """
    spotdl_rl class, which simplifies the process of downloading songs from Spotify.

    ```python
    from spotdl_rl import spotdl_rl

    spotdl_rl = spotdl_rl(client_id='your-client-id', client_secret='your-client-secret')

    songs = spotdl_rl.search(['joji - test drive',
        'https://open.spotify.com/track/4cOdK2wGLETKBW3PvgPWqT'])

    results = spotdl_rl.download_songs(songs)
    song, path = spotdl_rl. download(songs[0])
    ```
    """
    ...
    def __init__(
        ...
        """
        Initialize the spotdl_rl class
