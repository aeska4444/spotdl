...
from spotdl_rl.download.progress_handler import ProgressHandler
from spotdl_rl.providers.audio import AudioProvider, YouTube, YouTubeMusic
from spotdl_rl.providers.lyrics import AzLyrics, Genius, LyricsProvider, MusixMatch, Synced
from spotdl_rl.types.options import DownloaderOptionalOptions, DownloaderOptions
from spotdl_rl.types.song import Song
from spotdl_rl.utils.archive import Archive
from spotdl_rl.utils.config import (
    DOWNLOADER_OPTIONS,
    create_settings_type,
    get_errors_path,
    get_temp_path,
)
from spotdl_rl.utils.ffmpeg import FFmpegError, convert, get_ffmpeg_path
from spotdl_rl.utils.formatter import create_file_name
from spotdl_rl.utils.m3u import gen_m3u_files
from spotdl_rl.utils.metadata import MetadataError, embed_metadata
from spotdl_rl.utils.search import gather_known_songs, reinit_song, songs_from_albums

from spotdl_rl.providers.translate import TranslateProvider, Lyricstranslate,\
    Animesonglyrics, Googletranslate, YoutubeSbs
...
class Downloader:
    ...
    def __init__(
        ...
        # from spotdl_rl.types.options.DOWNLOADER_OPTIONS
        ...
        # try to use the spotdl_rl's ffmpeg
        ...
        # Initialize translate providers
        self.translate_providers: List[TranslateProvider] = []
        for translate_provider in self.settings["translate_providers"]:
            translate_class = TRANSLATE_PROVIDERS.get(translate_provider)
            if translate_class is None:
                raise DownloaderError(f"Invalid translate provider: {translate_provider}")

            self.translate_providers.append(translate_class())
        ...
        for translate_provider in self.translate_providers:
            ...
            if type(translate_provider) not in (Googletranslate,):
                translator = translate_provider.get_translate(
                    song.search_name, song.artists, song.url)
                # print(f"{translator=}")
                if translator[1]:
                    if translator[0]:
                        translation = f"{str(translate_provider.name)}" \
                                f"\n{translator[0]}" \
                                f"\n{translator[1]}"
                    else:
                        translation = f"{str(translate_provider.name)}" \
            ...
            if translation:
                logger.debug(
                    "Found translation for %s on %s", song.display_name, translate_provider.name
                )

                return translation

            logger.debug(
                "%s failed to find translation for %s",
    ...
    @staticmethod
    def lyrics_prep(lyrisc: str) -> Optional[str]:
