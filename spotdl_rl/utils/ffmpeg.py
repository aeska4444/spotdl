...
from spotdl_rl.utils.config import get_spotdl_rl_path
from spotdl_rl.utils.formatter import to_ms
...
def get_local_ffmpeg() -> Optional[Path]:
    ...
    ffmpeg_path = Path(get_spotdl_rl_path()) / (
...
def download_ffmpeg() -> Path:
    """
    Download ffmpeg binary to spotdl_rl directory.
    ...
    ffmpeg_path = Path(
        os.path.join(
            get_spotdl_rl_path(), "ffmpeg" + (".exe" if os_name == "windows" else "")
        )
    )
    ...
    # Download binary and save it to a file in spotdl_rl directory
