...
from spotdl_rl.download.downloader import Downloader
from spotdl_rl.download.progress_handler import ProgressHandler, SongTracker
from spotdl_rl.types.options import (
    DownloaderOptionalOptions,
    DownloaderOptions,
    WebOptions,
)
from spotdl_rl.types.song import Song
from spotdl_rl.utils.arguments import create_parser
from spotdl_rl.utils.config import (
    DOWNLOADER_OPTIONS,
    create_settings_type,
    get_spotdl_rl_path,
)
from spotdl_rl.utils.github import RateLimitError, get_latest_version, get_status
from spotdl_rl.utils.search import get_search_results
...
async def shutdown_event():
    ...
    if (
        ...
        sessions_dir = Path(get_spotdl_rl_path(), "web/sessions")
...
async def download_url(
    ...
    if state.web_settings.get("web_use_output_dir", False):
        ...
        client.downloader.settings["output"] = str(
            (get_spotdl_rl_path() / f"web/sessions/{client.client_id}").absolute()
        )
...
async def download_file(
    ...
    expected_path = str((get_spotdl_rl_path() / "web/sessions").absolute())
    if state.web_settings.get("web_use_output_dir", False):
        expected_path = str(
...
def fix_mime_types():
    ...
    incorrect values, which causes spotdl_rl to fail to render on
    the frontend.
    ...
    # https://github.com/spotdl_rl/spotify-downloader/issues/1540
    mimetypes.add_type("application/javascript", ".js")

    # Not known to be problematic, but used by spotdl_rl:
