...
Default config - spotdl_rl.utils.config.DEFAULT_CONFIG
"""
...
def get_spotdl_rl_path() -> Path:
    """
    Get the path to the spotdl_rl folder.

    ### Returns
    - The path to the spotdl_rl folder.

    ### Notes
    - If the spotdl_rl directory does not exist, it will be created.
    """

    # Check if os is linux
    if platform.system() == "Linux":
        # if platform is linux, and XDG DATA HOME spotdl_rl folder exists, use it
        user_data_dir = Path(platformdirs.user_data_dir("spotdl_rl", "spotdl_rl"))
        if user_data_dir.exists():
            return user_data_dir

    spotdl_rl_path = Path(os.path.expanduser("~"), ".spotdl_rl")
    if not spotdl_rl_path.exists():
        os.mkdir(spotdl_rl_path)

    return spotdl_rl_path
...
def get_config_file() -> Path:
    ...
    return get_spotdl_rl_path() / "config.json"
...
def get_cache_path() -> Path:
    ...
    return get_spotdl_rl_path() / ".spotipy"
...
def get_temp_path() -> Path:
    ...
    temp_path = get_spotdl_rl_path() / "temp"
...
def get_errors_path() -> Path:
    ...
    errors_path = get_spotdl_rl_path() / "errors"
...
def get_config() -> Dict[str, Any]:
    ...
    if not config_path.exists():
        raise ConfigError(
            ...
            "Please run `spotdl_rl --generate-config` to create a config file."
        )
...
DEFAULT_CONFIG: spotdl_rlOptions = {
