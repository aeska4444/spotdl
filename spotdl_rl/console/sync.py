...
def sync(
    ...
    if query and save_path:
        if any(req for req in query if req.endswith(".spotdl_rl")):
            # If the query contains a .spotdl_rl file, and we are about to create
            # .spotdl_rl file, raise an error.
            raise ValueError(
                "Cannot create a sync file with a .spotdl_rl file in the query."
            )
    ...
    if len(query) == 1 and query[0].endswith(".spotdl_rl") and not save_path:
        # Load the sync file
